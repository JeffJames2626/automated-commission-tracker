...
import { queueSummary, staleRouting, liveOpForCapture } from './repo/ops.mjs';
import { listGoals, mirrorCount } from './repo/mirror.mjs';
import { startRouting, resolveRoute, chooseRoute, onCaptureDeleted, APP } from './apps/routing.mjs';
import { confirmAppAction, APP_ACTION_KINDS, actionStatuses } from './apps/actions.mjs';
import { dreamView, routingForCapture, goalIdOf } from './apps/dreams.mjs';
...
export function createRouter(deps = {}) {
  ...
  const routes = {
    ...
    'DELETE item': async ({ user, d, query }) => {
      ...
      await onCaptureDeleted(d, user.id, String(query.id || ''));
    ...
    'POST action': async ({ user, d, body }) => {
      ...
      if (APP_ACTION_KINDS[a.kind]) {
        ...
        if (r.error) return json({ action: r.action || a, result: { note: r.error } }, r.action && r.action.status === 'failed' ? 200 : 409);
