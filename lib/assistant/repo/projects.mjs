...
export async function createProject(db, userId, { name, kind = 'project', description = null, emoji = null, aliases = [] }) {
  const clean = String(name || '').trim().slice(0, 80);
  if (!clean) return null;
  const existing = await findProjectByName(db, userId, clean);
  if (existing) return existing;
  const r = await db.query(`INSERT INTO asst_projects (id, user_id, name, kind, description, emoji, aliases)
      VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (user_id, lower(name)) DO NOTHING RETURNING *`,
    [newId('project'), userId, clean, PROJECT_KINDS.includes(kind) ? kind : 'project', description, emoji, JSON.stringify(aliases.slice(0, 12))]);
