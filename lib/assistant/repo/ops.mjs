...
// applied / rejected / cancelled / superseded   final
//
...
export const FINAL = ['applied', 'rejected', 'cancelled', 'superseded'];
...
export async function createOp(db, { id, userId, app, kind, captureId = null, actionId = null, dependsOn = null, targetId = null, payload = {}, status, trace = [] }) {
  ...
  const ex = captureId
    ? await db.query(`SELECT * FROM asst_app_ops WHERE user_id = $1 AND app = $2 AND capture_id = $3 AND status <> ALL($4) ORDER BY created_at DESC LIMIT 1`, [userId, app, captureId, ['rejected', 'cancelled', 'superseded']])
    : await db.query('SELECT * FROM asst_app_ops WHERE id = $1 AND user_id = $2', [opId, userId]);
  return { op: ex[0] || null, created: false };
}
...
export async function transition(db, userId, id, from, to, { payload, targetId, dependsOn, reason, result, traceEntry } = {}) {
  const r = await db.query(`UPDATE asst_app_ops SET status = $4,
        ...
        done_at = CASE WHEN $4 IN ('applied','rejected','cancelled','superseded') THEN now() ELSE done_at END,
    ...
    [id, userId, from, to, payload === undefined ? null : JSON.stringify(payload), targetId === undefined ? null : (targetId || ''),
      ...
      JSON.stringify(traceEntry ? [traceEntry] : [])]);
  return r[0] || null;
}
...
export async function countQueued(db, userId, app) {
  const r = await db.query(`SELECT count(*)::int AS n, max(created_at) AS newest FROM asst_app_ops WHERE user_id = $1 AND app = $2 AND status = 'queued'`, [userId, app]);
  return r[0];
}
...
export async function liveOpForCapture(db, userId, app, captureId) {
  const r = await db.query(`SELECT * FROM asst_app_ops WHERE user_id = $1 AND app = $2 AND capture_id = $3 AND status <> ALL($4)
    ORDER BY created_at DESC LIMIT 1`, [userId, app, captureId, ['rejected', 'cancelled', 'superseded']]);
  return r[0] || null;
}
...
// A confirmed change creates nothing; it is recognised by the record version
// it produced ("v:<record id>:<seq>").
export async function createdIdIndex(db, userId, app, sinceDays = 400) {
  const rows = await db.query(`SELECT id, kind, result FROM asst_app_ops WHERE user_id = $1 AND app = $2 AND status = 'applied'
    AND done_at > now() - ($3 || ' days')::interval`, [userId, app, String(sinceDays)]);
  const byId = new Map();
  for (const r of rows) {
    ...
    if (r.kind === 'update_goal' && res.record && res.seq != null) byId.set('v:' + res.record.id + ':' + res.seq, r.id);
  }
  return byId;
}
