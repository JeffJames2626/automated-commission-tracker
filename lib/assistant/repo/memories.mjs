...
// How a memory came to exist, so the assistant can weigh it: 'stated' (the
// owner typed it on the Memory screen), 'conversation' (they asked the
// assistant to remember it), 'extracted' (the classifier pulled it from a
// capture). 'inferred' is reserved: nothing writes it today.
const ORIGIN = { user: 'stated', user_edit: 'stated', conversation: 'conversation', capture: 'extracted' };
...
export async function addMemory(db, userId, { kind = 'fact', statement, subjectType = null, subjectId = null, sourceType = null, sourceId = null }) {
  ...
  const r = await db.query(`INSERT INTO asst_memories (id, user_id, kind, statement, subject_type, subject_id, source_type, source_id, origin)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id, kind, statement, subject_type, subject_id, source_type, source_id, origin, status, created_at`,
    [newId('memory'), userId, MEMORY_KINDS.includes(kind) ? kind : 'fact', s, subjectType, subjectId, sourceType, sourceId, ORIGIN[sourceType] || 'stated']);
  return r[0];
}

export async function listMemories(db, userId, { status = 'active', limit = 200 } = {}) {
  return db.query(`SELECT m.id, m.kind, m.statement, m.subject_type, m.subject_id, m.source_type, m.source_id, m.origin, m.status, m.created_at, m.updated_at,
...
export async function searchMemories(db, userId, q, limit = 10) {
  ...
  if (!text) return [];
  return db.query(`SELECT m.id, m.kind, m.statement, m.subject_type, m.subject_id, m.source_type, m.source_id, m.origin, m.created_at, p.name AS project_name,
