...
export async function applyClassification(db, userId, id, c) {
  const r = await db.query(`UPDATE asst_captures SET
      ...
      status = CASE WHEN $9::text IS NOT NULL AND classification_state <> 'manual' THEN $9 ELSE status END,
      ai = $10,
...
const EDITABLE = {
  ...
  status: v => STATUS_KEYS.includes(v) ? v : undefined,
...
export async function listCaptures(db, userId, { status, kind, projectId, limit = 40, before, includeQuestions = false, openTasks = false } = {}) {
  ...
  if (status === 'open') where.push("c.status NOT IN ('archived','built')");
  ...
  if (openTasks) where.push("c.kind IN ('task','reminder') AND c.completed_at IS NULL");
  if (before) {
...
export async function queryCaptures(db, userId, { kinds, statuses, projectId, dueFrom, dueTo, openOnly, staleDays, limit = 25, order = 'recent' } = {}) {
  ...
  if (openOnly) where.push("c.completed_at IS NULL AND c.status NOT IN ('archived','built')");
...
export async function pendingClassification(db, userId, limit = 5) {
  return db.query(`SELECT * FROM asst_captures WHERE user_id = $1 AND classification_state IN ('pending','failed')
      AND captured_at > now() - interval '30 days' ORDER BY captured_at DESC LIMIT $2`, [userId, limit]);
}

export async function counts(db, userId) {
  const r = await db.query(`SELECT
      count(*) FILTER (WHERE status = 'inbox' AND kind <> 'question')::int AS inbox,
      count(*) FILTER (WHERE kind IN ('task','reminder') AND completed_at IS NULL AND status <> 'archived')::int AS open_tasks,
      count(*) FILTER (WHERE kind <> 'question')::int AS total
    FROM asst_captures WHERE user_id = $1`, [userId]);
  return r[0];
}
