...
export async function listCaptures(db, userId, { status, kind, projectId, limit = 40, before, includeQuestions = false, openTasks = false } = {}) {
  ...
  // Journal entries have their own list; what was pulled out of them is in the inbox.
  if (kind !== 'journal') where.push("c.kind <> 'journal'");
...
export async function queryCaptures(db, userId, { kinds, statuses, projectId, dueFrom, dueTo, openOnly, staleDays, limit = 25, order = 'recent' } = {}) {
  ...
  if (!(kinds && kinds.includes('journal'))) where.push("c.kind <> 'journal'");
...
// Journal entries still to be gone through (including a run that crashed and
// left its claim behind).
export async function pendingJournal(db, userId, limit = 3) {
  return db.query(`SELECT * FROM asst_captures WHERE user_id = $1 AND kind = 'journal' AND (details->>'journal_state' = 'pending'
        OR (details->>'journal_state' = 'processing' AND (details->>'journal_claimed_at')::timestamptz < now() - interval '150 seconds'))
      AND captured_at > now() - interval '7 days' ORDER BY captured_at LIMIT $2`, [userId, limit]);
}
...
export async function counts(db, userId) {
  const r = await db.query(`SELECT
      count(*) FILTER (WHERE status = 'inbox' AND kind NOT IN ('question','journal'))::int AS inbox,
