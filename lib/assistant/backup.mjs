...
export async function dumpAll(db) {
  ...
  for (const t of TABLES) {
    ...
    out.tables[t] = await db.query(`SELECT ${cols.map(q).join(', ')} FROM ${t}`);
  }
  return out;
}
...
export async function dumpUser(db, userId) {
  ...
  for (const t of TABLES) {
    ...
    if (!key) continue;
    out.tables[t] = await db.query(`SELECT ${cols.map(q).join(', ')} FROM ${t} WHERE ${key} = $1`, [userId]);
  }
  return out;
}
...
export async function restore(db, dump) {
  ...
  for (const t of TABLES) {
    ...
    report[t] = { inserted, skipped: rows.length - inserted };
  }
  return report;
}
