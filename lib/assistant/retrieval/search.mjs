...
import { searchMirror, goalIdOf, dreamSource } from '../repo/mirror.mjs';
...
async function appGroups(db, userId, query, perSource) {
  ...
  return [{
    ...
    items: rows.map(g => {
      const was = (g.aliases || []).find(a => a.toLowerCase().includes(low));
      return Object.assign(dreamSource(goalIdOf(g), g.title, g.last_seen_at), {
        snippet: [g.status, g.data && g.data.category ? g.data.category.name : null, was ? 'formerly “' + was + '”' : null].filter(Boolean).join(' · '),
