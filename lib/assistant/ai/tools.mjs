  ...
  // ---------------- READ: the owner's own notes ----------------
  {
    ...
    async run(ctx, i, reg) {
      ...
      mems.forEach(m => {
        ...
        lines.push(`[${id}] Memory (${m.kind}${m.project_name ? ', ' + m.project_name : ''}, saved ${fmtDate(m.created_at, ctx.tz)}): ${m.statement}`);
      });
      return { text: listText('Notes and memories matching “' + i.query + '”', lines), count: lines.length };
    },
  },
  {
    name: 'list_my_items',
...
export function toolDefs(states) {
  return TOOLS.filter(t => !t.service || states[t.service] === 'connected')
