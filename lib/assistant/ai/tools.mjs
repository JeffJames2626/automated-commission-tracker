...
import { listGoals, searchMirror, getMirrorRow } from '../repo/mirror.mjs';
import { APP, matchGoals, startRouting, resolveRoute } from '../apps/routing.mjs';
import { dreamView, goalIdOf } from '../apps/dreams.mjs';
import { proposeAppAction } from '../apps/actions.mjs';
import { changesSince } from '../apps/digest.mjs';
import { sinceLines, dreamSource } from '../briefing.mjs';
...
// fenced so nothing in it can act as an instruction.
...
// A dream named in words → one goal, or the candidates to ask about.
async function resolveDream(ctx, name) {
  ...
  if (m.candidates.length === 1) return { id: m.candidates[0].id };
...
export const TOOLS = [
  // ---------------- READ: the owner's own notes ----------------
  {
    ...
    async run(ctx, i, reg) {
      ...
      if (dreams.length) lines.push(fence('Dream Board', dreams.map(g => `[${reg.add(dreamSource(goalIdOf(g), g.title, g.synced_at))}] Dream Board goal “${g.title}”${(g.aliases || []).length ? ' (formerly ' + g.aliases.join(', ') + ')' : ''} · ${g.status || 'no status'} — use get_dream for details`).join('\n')));
  ...
  },
  {
    ...
    async run(ctx, i, reg) {
      ...
      const lines = caps.map(c => formatNote(c, reg.add(noteItem(c), c.summary || c.raw_preview || c.title), ctx.tz));
      mems.forEach(m => { const id = reg.add({ provider: 'memory', kind: m.kind, recordId: m.id, title: m.statement, url: '#/memory', date: m.created_at }, m.statement); lines.push(`[${id}] Memory: ${m.statement}`); });
      ext.forEach(e => { const id = reg.add({ provider: e.provider, kind: e.kind, recordId: e.provider_record_id, title: e.title, url: e.url, date: e.occurred_at }); lines.push(`[${id}] Linked ${e.kind}: ${e.title} (${fmtDate(e.occurred_at, ctx.tz)})`); });
  ...
  // ---------------- INTERNAL writes ----------------
  {
    ...
    async run(ctx, i) {
      const { capture } = await createCapture(ctx.db, ctx.userId, { clientRef: 'assistant:' + newId('capture'), rawText: i.text, sourceType: 'assistant' });
      ...
      const updated = await applyClassification(ctx.db, ctx.userId, capture.id, {
        kind: i.kind || h.kind, title: h.title, projectId, dueAt: i.due_at && !isNaN(Date.parse(i.due_at)) ? new Date(i.due_at).toISOString() : h.dueAt,
        state: 'done', ai: { how: 'assistant' },
      });
  ...
  // ---------------- READ: Dream Board (the owner's dreams and goals) ----------------
  {
    ...
    async run(ctx, i, reg) {
      ...
      const lines = goals.slice(0, 40).map(g => {
        ...
        return `[${reg.add(dreamSource(goalIdOf(g), g.title, g.synced_at), [g.title, g.status, fieldsLine(d.fields)].filter(Boolean).join(' · '))}] “${g.title}” · ${g.status || 'no status'}${d.category ? ' · ' + d.category.name : ''}` +
  ...
  },
  {
    ...
    async run(ctx, i, reg) {
      ...
      if (!r.id) return { text: r.candidates.length ? 'Several dreams could match — ask the owner which one: ' + r.candidates.map(c => '“' + c.title + '”').join(', ') : 'No dream in Dream Board matches “' + i.dream + '”.', count: 0 };
      ...
      const cur = [
        ...
        ...(v.changes.length ? v.changes.map(c => `- ${fmtDate(c.at, ctx.tz)}: ${c.text}${c.by === 'assistant' ? ' (from the owner’s capture)' : ''}`) : ['- none']),
      ].filter(Boolean);
      const words = v.words.map(w => `[${reg.add({ provider: 'notes', kind: 'capture', recordId: w.id, title: w.title, url: '#/item/' + w.id, date: w.capturedAt }, w.text)}] ${fmtDate(w.capturedAt, ctx.tz)}: “${clip(w.text, 600)}”${w.attachments ? ' (+' + w.attachments + ' attachment' + (w.attachments > 1 ? 's' : '') + ')' : ''}`);
  ...
  },
  {
    ...
    async run(ctx, i) {
      const r = await resolveDream(ctx, i.dream);
      if (!r.id) return { text: r.candidates.length ? 'Ask the owner which dream: ' + r.candidates.map(c => '“' + c.title + '”').join(', ') : 'No dream matches “' + i.dream + '”.', count: 0 };
      ...
      return { text: `Prepared for the owner to confirm (NOT done): ${p.action.summary}. Tell them the card is below your answer.`, count: 1 };
    },
  },
];
