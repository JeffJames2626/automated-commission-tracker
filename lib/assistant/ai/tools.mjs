...
import { guardClassification, statesDecision, asksToRemember, isHedged } from './hedge.mjs';
...
export const TOOLS = [
  ...
  // ---------------- INTERNAL writes ----------------
  {
    ...
    async run(ctx, i) {
      ...
      // Tentative words stay an idea, whatever kind was asked for.
  ...
  },
  {
    ...
    async run(ctx, i) {
      ...
      const own = String(ctx.ownerText || '');
      const ok = i.kind === 'decision' ? statesDecision(own) : (asksToRemember(own) || statesDecision(own)) && !isHedged(own);
