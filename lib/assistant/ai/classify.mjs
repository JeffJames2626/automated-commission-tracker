...
import { guardClassification } from './hedge.mjs';
...
const CAPTURE_KINDS = KIND_KEYS.filter(k => k !== 'question' && k !== 'journal');
...
const SCHEMA = {
  ...
  required: ['intent', 'kind', 'title', 'summary', 'project_id', 'new_project_name', 'tags', 'people', 'due_at', 'next_action', 'status', 'memories', 'confidence', 'reason'],
  properties: {
    ...
    reason: { type: 'string', description: 'One short line: why this kind and project (shown to the owner when tuning)' },
  },
};
...
- Tentative wording ("maybe", "should we", "what if", "thinking about", "might") is an idea, never a decision, and never a memory. Only settled wording ("we decided", "going with") is a decision.
- When unsure, say so with a low confidence; the owner would rather sort it than find it misfiled.
...
export async function classifyCapture({ db, userId, claude, capture, tz = 'UTC', now = Date.now(), timeoutMs = 20000 }) {
  ...
  const safe = guardClassification(c, capture.raw_text);
  c = safe.c;
  ...
  const updated = await applyClassification(db, userId, capture.id, {
    ...
    ai: { how, model: c.model || null, confidence: c.confidence, reason: c.reason || null, guard: safe.guard.length ? safe.guard : undefined, at: new Date(now).toISOString(), error },
