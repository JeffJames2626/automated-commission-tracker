...
- Tentative wording ("maybe we should…", "what if…", "thinking about…") is an idea, never a decision or a memory. Never restate a tentative thought as settled.
...
- Use remember only for what they explicitly asked you to remember or a decision they stated. "Maybe we should…" is an idea (save_capture kind idea), not a decision.
...
export async function answer({ db, user, claude, google, conversationId, question, tz, now = Date.now(), emit = () => {}, journalId = null, journalDone = [], reuseUserMessageId = null }) {
  const conv = await getConversation(db, user.id, conversationId);
  if (!conv) throw Object.assign(new Error('conversation not found'), { status: 404 });
  // A journal retry answers the entry already in the conversation instead of
  // adding it a second time.
  const earlier = reuseUserMessageId ? (conv.messages || []).find(m => m.id === reuseUserMessageId && m.role === 'user') : null;
  if (earlier) conv.messages = conv.messages.filter(m => m.id !== earlier.id);
  const lastAssistantText = ((conv.messages || []).filter(m => m.role === 'assistant').pop() || {}).content || '';
  const userMsg = earlier || await addMessage(db, user.id, conversationId, { role: 'user', content: question });
  emit({ type: 'user_message', message: userMsg });

  const reg = new SourceRegistry();
  const ctx = { db, userId: user.id, user, tz, now, google, conversationId, actions: [], created: [], journalId, ownerText: question, lastAssistantText };
  ...
  if (!claude) {
    ...
    const system = [
      ...
      // A retry after a failure part-way: what was already saved stays saved.
      .concat(journalId && journalDone.length ? [{ type: 'text', text: 'Already saved from this entry on an earlier attempt (do not save these again): ' + journalDone.map(t => '"' + String(t).slice(0, 80) + '"').join('; ') }] : []);
