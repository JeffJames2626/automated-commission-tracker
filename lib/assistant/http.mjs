...
export async function toRequest(req) {
  ...
  return {
    ...
    // Trimmed: a stray (encoded) space from a rewrite must never change the route.
    route: String(query.r || (url.pathname.match(/^\/api\/assistant\/(.+)$/) || [])[1] || '').trim().replace(/^\/+|\/+$/g, ''),
    query,
    headers: req.headers,
...
export async function sendResponse(res, out) {
  ...
  if (out.stream) {
    ...
    catch (e) { write({ type: 'error', error: publicError(e), ref: e && e.ref }); }
    return res.end();
  }
  if (out.binary) {
    res.statusCode = out.status || 200;
