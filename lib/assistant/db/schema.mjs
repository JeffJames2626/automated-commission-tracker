...
export const STATEMENTS = [
  ...
  `CREATE TABLE IF NOT EXISTS asst_apps (
    ...
    last_seen_at TIMESTAMPTZ, sync_until TIMESTAMPTZ,
    last_error TEXT,
    created_at ${TS}, updated_at ${TS},
    UNIQUE (user_id, app))`,
  ...
  // per app; rejected/cancelled rows stay as routing history.
  ...
  `CREATE UNIQUE INDEX IF NOT EXISTS asst_app_ops_live ON asst_app_ops (user_id, app, capture_id)
    WHERE capture_id IS NOT NULL AND status NOT IN ('rejected','cancelled')`,
  ...
  // One row per (record, version); a change caused by one of our own ops
  // carries that op's id (changes[].op_id).
  ...
  `CREATE INDEX IF NOT EXISTS asst_events_record ON asst_events (external_id, occurred_at)`,
  ...
  // "Connect Personal Assistant" started from the app (docs/assistant/
  // DREAM-BOARD-CONNECTOR.md): the app waits with a secret (stored hashed)
  // while the owner signs in with Google and approves in the browser. The
  // user_code is shown on both screens so the owner can check they match.
  `CREATE TABLE IF NOT EXISTS asst_app_links (
    id TEXT PRIMARY KEY,
    app TEXT NOT NULL,
    ...
    user_id TEXT REFERENCES asst_users(id) ON DELETE CASCADE,
    pair_code_enc TEXT,
    expires_at TIMESTAMPTZ NOT NULL,
    decided_at TIMESTAMPTZ,
    created_at ${TS})`,
  `CREATE INDEX IF NOT EXISTS asst_app_links_code ON asst_app_links (user_code, status)`,
];
