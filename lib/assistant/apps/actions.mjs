...
import { proposeAction, getAction } from '../repo/external.mjs';
...
function coerce(field, v) {
  ...
  // Statuses are compared the way the mirror stores them (lowercase).
  return (field === 'status' ? s.toLowerCase() : s) || undefined;
}
...
export async function proposeAppAction(db, userId, { kind, goalId, field, value, milestone }) {
  ...
  return { action: await proposeAction(db, userId, { kind, summary, payload }) };
}
...
export async function confirmAppAction(db, userId, action) {
  ...
  if (action.status === 'proposed') {
    ...
    const r = await db.query(`UPDATE asst_actions SET status = 'queued', decided_at = now()
      WHERE id = $1 AND user_id = $2 AND status = 'proposed' RETURNING *`, [action.id, userId]);
...
// — unless the board's history was rewound since, which ends the wait.
export async function verifyAction(db, userId, op) {
  ...
  const row = await getMirrorRow(db, userId, APP, 'goal', p.target.id);
  const res = op.result || {};
  const sameEpoch = !res.epoch || (row && row.app_epoch === res.epoch);
  if (!row || row.app_seq == null || (sameEpoch && res.seq != null && Number(row.app_seq) < res.seq)) return false;
  let ok, shows = null;
...
export async function verifyPending(db, userId, app) {
  const rows = await db.query(`SELECT o.* FROM asst_app_ops o JOIN asst_actions a ON a.id = o.action_id AND a.user_id = o.user_id
    WHERE o.user_id = $1 AND o.app = $2 AND o.status = 'applied' AND a.status = 'queued' ORDER BY o.done_at LIMIT 20`, [userId, app]);
