...
import { listGoals, eventsSince } from '../repo/mirror.mjs';
import { recentOps, queueSummary } from '../repo/ops.mjs';
import { describeChange, fmtAmount, isAchieved } from './records.mjs';
import { goalIdOf } from './dreams.mjs';
...
export async function dreamHighlights(db, userId, now = Date.now()) {
  ...
  ev.forEach(e => (e.changes || []).forEach(c => {
    if (c.kind !== 'milestone_completed' && c.kind !== 'status_changed' && c.kind !== 'updated') return;
  ...
  for (const g of goals) {
    ...
    items.push({ id: goalIdOf(g), title: g.title, line: bits.join(' · '), kind: isAchieved(g.status) ? 'achieved' : 'progress' });
  }
  ...
  if (quiet) items.push({ id: goalIdOf(quiet.g), title: quiet.g.title, line: `no update in ${Math.floor((now - quiet.last) / DAY)} days`, kind: 'quiet' });
  const choose = await db.query(`SELECT count(*)::int AS n FROM asst_app_ops WHERE user_id = $1 AND app = $2 AND status = 'needs_choice'`, [userId, APP]);
  return { items: items.slice(0, 4), needsChoice: choose[0].n, freshness: freshness(app, now) };
}
...
export async function changesSince(db, userId, sinceIso) {
  const [caps, events, ops, waiting] = await Promise.all([
    db.query(`SELECT c.id, c.kind, c.status, c.title, c.captured_at, c.project_id, p.name AS project_name,
      ...
      WHERE c.user_id = $1 AND c.captured_at >= $2 AND c.kind <> 'question' ORDER BY c.captured_at`, [userId, sinceIso]),
    eventsSince(db, userId, sinceIso, { app: APP }),
