...
export function createConnector({ db, config, now, limits }) {
  ...
  async function linkPoll(req, d) {
    ...
    if (!/^dbl_[A-Za-z0-9_-]{43}$/.test(device)) throw new HttpError(400, 'invalid');
    // Per link (what one board waiting needs), plus a loose ceiling per caller.
    await checkRate(d, 'linkpoll:' + sha256(device).slice(0, 24), 'linkpoll', now(), limits);
    await checkRate(d, 'linkpollip:' + callerIp(req), 'linkpollip', now(), limits);
