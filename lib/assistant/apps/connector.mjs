...
export function createConnector({ db, config, now, limits }) {
  async function authed(req, d) {
    ...
    if (!user || (user.session_epoch || 0) !== row.session_epoch || !(await isAllowedEmail(d, user.email, config().allowedEmails))) {
  ...
  async function pair(req, d) {
    ...
    const ip = String(req.headers['x-real-ip'] || String(req.headers['x-forwarded-for'] || '').split(',')[0] || 'unknown').trim().slice(0, 64);
    await checkRate(d, 'pair:' + ip, 'pair', now(), limits);
