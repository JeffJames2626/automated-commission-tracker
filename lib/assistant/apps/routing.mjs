...
export function dreamTitleFromWords(text) {
  ...
  let t = stripBoardWords(String(text || '').replace(filler, '')).replace(filler, '');
  ...
  const small = /^(a|an|the|of|for|and|or|to|in|on|at|with|by)$/i;
  return words.map((w, i) => (i === 0 || !small.test(w) ? w[0].toUpperCase() + w.slice(1) : w.toLowerCase())).join(' ');
}
