import { createOp, getOp, transition, stage, pendingCreates, dependents, staleRouting, waitingOnSync, appliedUnlinked,
  liveOpForCapture, cancelLiveOpsForCapture, setLinked, appendTrace, LIVE } from '../repo/ops.mjs';
import { getApp } from '../repo/apps.mjs';
import { listGoals, ensurePlaceholder, getMirrorRow } from '../repo/mirror.mjs';
import { getCapture } from '../repo/captures.mjs';
import { link } from '../repo/graph.mjs';
import { isAchieved } from './records.mjs';
...
async function goalChoices(db, userId) {
  const goals = (await listGoals(db, userId, APP)).map(r => ({
    kind: 'goal', id: r.provider_record_id.split(':').slice(1).join(':'), title: r.title, aliases: r.aliases || [],
    inactive: isAchieved(r.status) || r.status === 'archived', status: r.status,
  }));
  const pending = (await pendingCreates(db, userId, APP)).filter(o => o.payload && o.payload.title)
...
async function setCaptureStatus(db, userId, captureId, status, onlyFrom) {
  if (!captureId) return;
  await db.query(`UPDATE asst_captures SET status = $3, updated_at = now() WHERE id = $1 AND user_id = $2 AND status = ANY($4)`,
    [captureId, userId, status, onlyFrom]);
}
...
export async function resolveRoute(db, { userId, op }) {
  ...
  if (intent.mode === 'attach') {
    ...
    if (hit && hit.kind === 'goal') {
      const payload = await buildPayload(db, userId, capture, { goal_id: hit.id, goal_title: hit.title, intent });
      const r = await transition(db, userId, op.id, ['routing'], 'queued', { payload, targetId: hit.id, traceEntry: stage('queued', { target: hit.title }) });
      if (r) await setCaptureStatus(db, userId, capture.id, 'filed', ['inbox', 'thinking', 'maybe', 'active']);
      return r;
    }
    ...
    if (!ready) {
      return transition(db, userId, op.id, ['routing'], 'waiting', { payload: { intent }, traceEntry: stage('waiting', { on: 'first_sync' }) });
    }
    return needsChoice(db, userId, op, intent, m.candidates, titleCase(intent.target));
  }

  const { title, title_origin } = titleFor(capture, intent);
  if (intent.mode === 'item') {
    const payload = await buildPayload(db, userId, capture, { title, title_origin, intent });
    const r = await transition(db, userId, op.id, ['routing'], 'queued', { payload, traceEntry: stage('queued', { as: 'unsorted item' }) });
    if (r) await setCaptureStatus(db, userId, capture.id, 'filed', ['inbox', 'thinking', 'maybe', 'active']);
    return r;
  }
  ...
  const payload = await buildPayload(db, userId, capture, { title, title_origin, intent });
  const r = await transition(db, userId, op.id, ['routing'], 'queued', { payload, traceEntry: stage('queued', { title }) });
  if (r) await setCaptureStatus(db, userId, capture.id, 'filed', ['inbox', 'thinking', 'maybe', 'active']);
  return r;
}
...
async function needsChoice(db, userId, op, intent, candidates, suggestedTitle) {
  ...
  return transition(db, userId, op.id, ['routing', 'waiting'], 'needs_choice', { payload, traceEntry: stage('needs_choice', { candidates: payload.candidates.length }) });
}
...
export async function chooseRoute(db, { userId, captureId, choice, title }) {
  ...
  if (choice === 'keep') {
    const r = await transition(db, userId, op.id, from, 'cancelled', { reason: 'kept_here', traceEntry: stage('cancelled', { reason: 'kept_here' }) });
    await setCaptureStatus(db, userId, captureId, 'inbox', ['filed']);
    return { op: r };
  }
  ...
  if (choice && typeof choice === 'object' && choice.goal) {
    ...
    if (!row || row.deleted_at || row.missing_at) return { error: 'unknown_goal' };
    const payload = await buildPayload(db, userId, capture, { goal_id: String(choice.goal), goal_title: row.title, intent });
    const r = await transition(db, userId, op.id, from, 'queued', { payload, targetId: String(choice.goal), traceEntry: stage('queued', { target: row.title, chosen: true }) });
    await db.query('UPDATE asst_app_ops SET kind = $2 WHERE id = $1', [op.id, 'attach']);
    await setCaptureStatus(db, userId, captureId, 'filed', ['inbox', 'thinking', 'maybe', 'active']);
    return { op: r && Object.assign(r, { kind: 'attach' }) };
  }
  if (choice && typeof choice === 'object' && choice.pending) {
    const dep = await getOp(db, userId, String(choice.pending));
    if (!dep || dep.kind !== 'create_goal' || !LIVE.includes(dep.status)) return { error: 'unknown_goal' };
    const payload = await buildPayload(db, userId, capture, { goal_title: dep.payload.title, intent });
    const r = await transition(db, userId, op.id, from, 'waiting', { payload, dependsOn: dep.id, traceEntry: stage('waiting', { on: 'pending_create', chosen: true }) });
    await db.query('UPDATE asst_app_ops SET kind = $2 WHERE id = $1', [op.id, 'attach']);
    await setCaptureStatus(db, userId, captureId, 'filed', ['inbox', 'thinking', 'maybe', 'active']);
    return { op: r };
  }
  if (choice === 'new' || choice === 'item') {
    ...
    const r = await transition(db, userId, op.id, from, 'queued', { payload, targetId: '', dependsOn: '', traceEntry: stage('queued', { as: choice === 'new' ? 'new dream' : 'unsorted item', chosen: true }) });
    await db.query('UPDATE asst_app_ops SET kind = $2 WHERE id = $1', [op.id, choice === 'new' ? 'create_goal' : 'add_item']);
    await setCaptureStatus(db, userId, captureId, 'filed', ['inbox', 'thinking', 'maybe', 'active']);
    return { op: r };
  }
...
export async function onCaptureDeleted(db, userId, captureId) {
  return cancelLiveOpsForCapture(db, userId, captureId, 'capture_deleted');
}
...
export async function handleResult(db, { userId, app, result }) {
  ...
  if (result.status === 'applied') {
    ...
    const recordId = typeof rec.id === 'string' ? rec.id.slice(0, 120) : null;
    const clean = {
      record: recordId ? { type: rec.type === 'item' ? 'item' : 'goal', id: recordId } : null,
      created: sanitizeCreated(result.created), seq: Number.isSafeInteger(Number(result.seq)) ? Number(result.seq) : null,
    };
    let row = op;
    if (op.status !== 'applied') {
      row = await transition(db, userId, op.id, LIVE.concat(['cancelled', 'rejected', 'superseded']), 'applied', {
  ...
  if (result.status === 'rejected') {
    ...
    const reason = String(result.reason || 'invalid').slice(0, 40);
    ...
    if (r.capture_id) {
      await setCaptureStatus(db, userId, r.capture_id, 'inbox', ['filed']);
      ...
      if (['target_deleted', 'target_missing'].includes(reason) && r.kind === 'attach') {
        ...
        if (again && again.status === 'routing') {
          const cands = await goalChoices(db, userId);
          await needsChoice(db, userId, again, again.payload.intent || {}, matchGoals((r.payload.intent || {}).target || r.payload.goal_title || '', cands).candidates, (r.payload.intent || {}).target || '');
        }
      }
    }
    for (const d of await dependents(db, userId, r.id)) {
      const cands = await goalChoices(db, userId);
      await needsChoice(db, userId, d, d.payload.intent || {}, matchGoals((d.payload.intent || {}).target || '', cands).candidates, (d.payload.intent || {}).target || '');
    }
    return { ok: true };
  }
...
function sanitizeCreated(c) {
  ...
  const id = v => (typeof v === 'string' && /^[A-Za-z0-9_:.\-]{1,120}$/.test(v) ? v : null);
...
export async function finishApplied(db, userId, op) {
  if (!op || op.status !== 'applied') return;
  const res = op.result || {};
  const goalId = (res.created && res.created.goal) || (res.record && res.record.type === 'goal' ? res.record.id : null) || op.target_id;
  if (goalId) {
    ...
    if (op.kind === 'create_goal') {
      for (const d of await dependents(db, userId, op.id)) {
        const payload = Object.assign({}, d.payload, { goal_id: goalId });
        await transition(db, userId, d.id, ['waiting'], 'queued', { payload, targetId: goalId, dependsOn: '', traceEntry: stage('queued', { after: 'create applied' }) });
      }
    }
  }
  if (op.capture_id) {
    await setCaptureStatus(db, userId, op.capture_id, 'filed', ['inbox', 'thinking', 'maybe', 'active']);
...
// anything a crash or a rate limit left half-done.
