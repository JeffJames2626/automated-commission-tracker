...
export async function findLink(db, code) {
  ...
  const r = await db.query(`SELECT * FROM asst_app_links WHERE user_code = $1 ORDER BY created_at DESC LIMIT 1`, [c]);
  const row = r[0];
  if (!row) return null;
  if (row.status === 'pending' && new Date(row.expires_at).getTime() < Date.now()) row.status = 'expired';
  return row;
}
...
export async function decideLink(db, userId, code, allow, tokenKey) {
  ...
  if (!row || row.status !== 'pending') return { error: row ? row.status : 'not_found' };
  if (!allow) {
  ...
  if (!claim.length) return { error: 'decided' };
...
export async function pollLink(db, device, tokenKey) {
  ...
  if (row.status === 'approved') {
    ...
    const out = code ? await redeemPairing(db, { app: row.app, code, instanceId: row.instance_id, instanceLabel: row.instance_label, linkTemplate: row.link_template }) : null;
    if (!out) return { status: 'expired' };
    return { status: 'approved', token: out.token, app: out.app };
  }
  if (row.status === 'pending' || row.status === 'approving') {
    return new Date(row.expires_at).getTime() < Date.now() ? { status: 'expired' } : { status: 'pending' };
  }
