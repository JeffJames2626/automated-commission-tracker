import { changeKey } from '../repo/ops.mjs';
...
// Text Postgres will always accept: no NUL, no broken surrogate pair.
export const str = (v, n) => (typeof v === 'string' ? v.replace(/\u0000/g, '').slice(0, n).toWellFormed() : null);
const iso = v => {
  const t = typeof v === 'string' ? Date.parse(v) : NaN;
  return t >= -62135596800000 && t <= 253402300799999 ? new Date(t).toISOString() : null;   // years 1–9999
};
...
export function normalizeRecord(raw) {
  ...
  if (raw.fields && typeof raw.fields === 'object') {
    for (const [k, v] of Object.entries(raw.fields).slice(0, 30)) {
      ...
      else if (typeof v === 'string') fields[k] = str(v, 300);
    }
  }
...
const clipText = s => (typeof s === 'string' && s.length > 200 ? str(s, 199) + '…' : s);

// Compare the stored snapshot with a newer one. `prev` is the stored data
// (or null the first time), `createdBy` maps entity ids — and values set by
// confirmed changes — to the op that caused them (our own writes coming back).
// Returns { changes, progress }.
export function diffRecords(prev, next, { createdBy = new Map(), receivedAt = new Date().toISOString() } = {}) {
  ...
  if (!prev || prev.placeholder) {
    if (next.deleted) return { changes: [], progress: false };
    ...
    const own = (field, value) => createdBy.get(changeKey(next.id, field, value)) || null;     // a confirmed change of ours
    if (prev.deleted) push({ kind: 'restored', title: next.title, at: receivedAt });
    if (prev.title !== next.title) push({ kind: 'renamed', from: prev.title, to: next.title, at: at('title'), op_id: own('title', next.title) });
    if ((prev.status || null) !== (next.status || null)) push({ kind: 'status_changed', from: prev.status || null, to: next.status || null, at: at('status'), op_id: own('status', next.status) });
    ...
    for (const k of keys) {
      ...
      if (a !== b && !(a == null && b == null)) push({ kind: 'updated', field: k, from: a ?? null, to: b ?? null, at: at(k), op_id: own(k, b) });
    }
  ...
  return { changes, progress: changes.some(c => isProgress(c)) };
}
...
// whatever the template says, the result must stay on the confirmed origin.
export function canonicalLink(appRow, recordId) {
  if (!appRow || !appRow.base_url || !recordId || !appRow.link_template || !appRow.link_template.includes('{id}')) return null;
  try {
    const base = new URL(appRow.base_url);
    const u = new URL(appRow.link_template.replace('{id}', encodeURIComponent(recordId)), base);
