...
function paint(main, { item: c, related }) {
  ...
  main.innerHTML = `
    ...
    <article class="idea">
      ...
      <div class="seg" role="radiogroup" aria-label="Status">${Object.entries(statuses).map(([k, v]) => `<button class="${c.status === k ? 'on' : ''}" data-status="${k}" role="radio" aria-checked="${c.status === k}">${esc(v)}</button>`).join('')}</div>
  ...
  main.querySelector('[data-kind]').onchange = e => save({ kind: e.target.value }).then(r => r && paint(main, { item: r, related }));
  ...
  if (done) done.onchange = () => save({ completed_at: done.checked ? new Date().toISOString() : null }).then(r => r && paint(main, { item: r, related }));
